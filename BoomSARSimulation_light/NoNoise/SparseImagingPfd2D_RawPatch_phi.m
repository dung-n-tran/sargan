function [Y_hat] = SparseImagingPfd2D(S, s0, t0, fs, tAcq, ...
    tx_x, tx_y, tx_z, rx_x, rx_y, rx_z, grid_x, grid_y, grid_z, threshold, K, Noverlap)
%SparseImagingPfd2D_RawPatch_phi : Same as parseImagingPfd2D_RawPatch, but uses a random measurement matrix (phi*S = phi*D*x)
%
%Input parameters
...
while 1
    ...
    Y = phi*S(:, i:j);
    D = zeros(size(Y, 2)*M, NumGrid);
    makeDictionary_1(phi, tx_x(i:j), tx_y(i:j), tx_z(i:j), rx_x(i:j), rx_y(i:j), rx_z(i:j));
    X = omp_S(D, Y(:), threshold);
    %X = SolveOMPF(D, Y(:), 'isNonnegative', false);
    ...
    function [D] = makeDictionary(phi, tx_x, tx_y, tx_z, rx_x, rx_y, rx_z)
        ...
        for n = 1:NumGrid
            ...
            data = phi*generateTDRawData(tx_x, tx_y, tx_z, rx_x, rx_y, rx_z, p_target, s0, t0, fs, tAcq);
                   
            D(:, n) = data(:);
        end
    end

    function[D] = makeDictionary_1(phi, tx_x, tx_y, tx_z, rx_x, rx_y, rx_z)
        %MAKEDICTIONARY : Makes dictionary for denoising SAR images (the parabolas). An
        % atom in the dictionary is the raw data corresponding to a point
        % on the grid.
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        ref_dat = generateTDRawData(tx_x, tx_y, tx_z, rx_x, rx_y, rx_z, [grid_x(1) grid_y(1) grid_z(1) 1], s0, t0, fs, tAcq);
        dat_fft = fft(ref_dat);
        ...
        for n = 2:NumGrid
            ...
            data = phi*shiftSig(dat_fft, (td-ref_t)', fs);
            D(:, n) = data(:);
        end
    end
