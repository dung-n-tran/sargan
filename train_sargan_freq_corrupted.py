...
import pickle
from sar_data_utilities import DATA_PATH, load_sar_dict
...
# experiment_name = "various_freq_corrupted_real_det_0_1_coef_range_dict_dist_5"
# experiment_name = "5_freq_corrupted_real_det_0_1_coef_range_dict_dist_20"
experiment_name = "5_freq_corrupted_real_det_0_1_coef_range_dict_dist_5"
...
#                 cond = add_gaussian_noise(cond, sd=noise_standard_deviation)   
                missing_idx = np.random.randint(0, n_missing_rates)
                freq_gap = freq_gaps[missing_idx]
                cond, _ = insert_freq_gaps(cond, (ori_n_samples, ori_n_apertures), freq_gap)
...
#                 plt.savefig(os.path.join(output_path, '%s_training_losses_epoch_%s.jpg' % (experiment_name, epoch)))
                ...
                img = scene_raw_data_batch[0:batch_size, :, :]
                cond = img.copy()
                missing_idx = np.random.randint(0, n_missing_rates)
                freq_gap = freq_gaps[missing_idx]
                pcond, _ = insert_freq_gaps(cond, (ori_n_samples, ori_n_apertures), freq_gap)
                pimg, pcond = preprocess_test(img, pcond, (ori_n_samples, ori_n_apertures), DOWNSAMPLE, downsample_factor)                    
                gen_img = sess.run(model.gen_img, feed_dict={model.image:pimg, model.cond:pcond})
                det_in_snr, det_out_snr, det_snr_gain = snr(pimg, pcond, gen_img, norm_flag=0, mag_flag=0)
                det_snr_gains.append(det_snr_gain)
                ...
                ax = fig.add_subplot(132)
                ax = visualize_raw_data(pcond[0, :, :, 0], ax, db_range)
                ax.set_title("Corrupted SNR = {:.2f} [dB]".format(det_in_snr), color='grey')
                ax = fig.add_subplot(133)
                ax = visualize_raw_data(gen_img[0, :, :, 0], ax, db_range)
                ax.set_title("Reconstructed SNR = {:.2f} [dB]".format(det_out_snr), color='grey')
                plt.tight_layout()
                test_filename_raw_det = os.path.join(output_path, '%s_epoch_%s_raw_batchsize_%s_det.jpg' % (experiment_name, epoch, batch_size))
                plt.savefig(test_filename_raw_det, dpi=300)
                ...
                legend = ax.legend(["Original", "Corrupted", "Reconstructed"]); plt.setp(legend.get_texts(), color='grey')
                ax.set_xlim([0, pimg[0, :, line_idx, 0].shape[0]])
                test_filename_one_aperture_det = os.path.join(output_path, '%s_epoch_%s_one_aperture_batchsize_%s_det.jpg' % (experiment_name, epoch, batch_size))
                plt.savefig(test_filename_one_aperture_det, dpi=300)
                

                # Deterministic SNR
                fig = plt.figure(figsize=(16, 9));
                ax = fig.add_subplot(111)
                ax.plot(det_snr_gains); ax.set_title("PSNR Gain", color='grey'); ax.set_xlabel("Epoch")
                test_filename_snr_gain_det = os.path.join(output_path, '%s_epoch_%s_gain_batchsize_%s_det.jpg' % (experiment_name, epoch, batch_size))
                plt.savefig(test_filename_snr_gain_det, dpi=300)
                ...
                ax.set_xlabel("Frequency [MHz]"); ax.set_ylabel("[dB]")
                ax.fill_between(freq[0:n_fft // zoom_factor], corrupted_spectrum_dB, y_min, color=corrupted_color, alpha=0.1)
                test_filename_spectrum_det = os.path.join(output_path, '%s_epoch_%s_spectrum_batchsize_%s_det.jpg' % (experiment_name, epoch, batch_size))
                plt.savefig(test_filename_spectrum_det, dpi=300)
                ...
                attachments = [
                    ...
                    test_filename_raw_det,
                    test_filename_one_aperture_det,
                    test_filename_snr_gain_det,
                    test_filename_spectrum_det
                ]
                send_images_via_email(email_alert_subject,
                                     email_alert_text,
                                     attachments,
                                     sender_email="ozawamariajp@gmail.com", 
                                     recipient_emails=["helmeteam@gmail.com"])
                plt.close("all")
...
if __name__=="__main__":
    ...
    parser.add_argument("--dict", "-d", help="SAR dictionary type", default="coarse")
